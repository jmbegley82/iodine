...
public:
	...
	void UpdatePosition(double ms);		//!< Update current position based on _velocity
	...
	coords PredictNextPosition(double ms);	//!< Calculate next position (collision notwithstanding)
	double PredictNextPositionX(double ms);	//!< Calculate next X position
	double PredictNextPositionY(double ms);	//!< Calculate next Y position
#if defined DEBUG
